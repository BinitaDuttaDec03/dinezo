...
export const addToCart = TryCatch(async (req: IAuthenticatedRequest, res) => {
    if (!req.user) {
...
export const fetchMyCart = TryCatch(async (req: IAuthenticatedRequest, res) => {
    if (!req.user) {
        return res.status(401).json({
            message: "please login",
        });
...
export const clearCart = TryCatch(async (req: IAuthenticatedRequest, res) => {
    const userId = req.user?._id;
    if (!userId) {
        return res.status(401).json({
