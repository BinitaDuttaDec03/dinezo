...
const AddAddressPage = () => {
  ...
  const fetchAddresses = async () => {
    try {
      const { data } = await axios.get(`${import.meta.env.VITE_RESTAURANT_SERVICE_URL}/api/address/all`, {
        headers: {
  ...
  const deleteAddress = async (id: string) => {
    ...
    try {
      setDeletingId(id);
      await axios.delete(`${import.meta.env.VITE_RESTAURANT_SERVICE_URL}/api/address/${id}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
      });
